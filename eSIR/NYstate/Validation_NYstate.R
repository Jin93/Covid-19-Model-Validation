...
###### We therefore switch to the state-level data from New York Times data source for our analysis on New York state:
...
change_time=c('New York' = c("03/18/2020"))
...
start.date = c('New York' = '3.16.20') # first day in the training data set
...
dat.length = which(names(F.region)=='5.2.20') # only report observed data until this date
...
R.region = R.region[1:dat.length]
...
names(I.region) = names(F.region) = names(R.region) = names.rawdata[1:dat.length]
...
#candidates = expand.grid(pi.min=seq(0.06,0.15,by=0.01),lambda0=seq(0.035,0.07,by=0.005)) 
...
write.csv(output,file=paste0('eSIR/NYState/',region.name,'-eSIR-',lag-lag.val,'days_before_peak.csv'))
