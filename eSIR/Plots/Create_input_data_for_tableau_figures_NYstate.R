...
region = 'NYstate'
lag.val = 7
lags = c(14,7) # for NYstate
for (i in 1:length(lags)){
  dat.tables[[i]] = read.csv(paste0('eSIR/',region,'/',region.name,'-eSIR-',lag-lag.val,'days_before_peak.csv'))
}

validation.indx = c('7 days before peak','Till peak')
out = list()
