...
d_italy=d_italy[which(as.Date(d_italy$DateRep,format="%d/%m/%Y") < as.Date("2020-03-14",format="%Y-%m-%d")),]
...
last_date_training_NY="2020-03-24"
...
# fit = sampling(m,data=stan_data,iter=50,warmup=25,chains=2)
# niter_notburning=50
# niter_notburning=100
# # 
...
# 
# # fit = sampling(m,data=stan_data,iter=40,warmup=20,chains=4,thin=4,control = list(adapt_delta = 0.95, max_treedepth = 10))
# set.seed(111)
# if(DEBUG) {
#   fit = sampling(m,data=stan_data,iter=100,warmup=50,chains=2)
#   #fit = sampling(m,data=stan_data,iter=150,warmup=75,chains=2)
#   
#   #fit = sampling(m,data=stan_data,iter=200,warmup=100,chains=2)
...
save(out_cvd19_model,file="output/out_cvd19_model_italy_ny_2week_constraint_4chains.RData")
# 
...
data_sourse_='jhu'
dir_figure="fig/ita_ny_2week_constraint_4chains/"
...
for(i in 2){#italy
  ...
  write.csv(table,file=paste0(dir_fig,"italy_2week_constraint_4chains.csv"))
}

ylim_=range(as.numeric(c(table_est_death,
                         table_l_est_death,table_h_est_death)))
par(mar=c(1,1,1,1))
pdf(paste0(dir_fig,"italy2week_constrain_4chains.pdf"))
...
for(i in 14){#NY
  ...
  write.csv(table,file=paste0(dir_fig,"NY_2week_constraint_4chains.csv"))
  
  ylim_=range(as.numeric(c(table_est_death,
                           table_l_est_death,table_h_est_death,table_true_death[(table_true_death)!="NA"])))
  par(mar=c(1,1,1,1))
  pdf(paste0(dir_fig,country,"_",data_sourse,"_2week_NY_constraint_4chains.pdf"))
  plot(table_true_death[(table_true_death)!="NA"],ylim=ylim_,xlim=c(0,length(table_est_death)))
  lines(table_est_death,col="red")
  lines(table_l_est_death,col="red")
  lines(table_h_est_death,col="red")
