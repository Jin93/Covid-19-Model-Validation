...
save(out_cvd19_model,file="/Users/yiwang/Dropbox/cvd/cvd_N/model/output/April_21/out_cvd19_model_jhuData_apiData_india_NY_tillpeak_projection.RData")


### plot, jhu data
data_sourse_='jhu'
dir_figure="/Users/yiwang/Dropbox/cvd/cvd_N/model/fig/April_24_india_NY_tillpeak_projection/"
dir.create(dir_figure)
load("/Users/yiwang/Dropbox/cvd/cvd_N/model/output/April_21/out_cvd19_model_jhuData_apiData_india_NY_tillpeak_projection.RData")
...
#get_plot<-function(out_model,data_sourse,dir_fig,name_data_sourse,countries){
  ...
  for(i in 16){#NY
    ...
    write.csv(table,file=paste0(dir_fig,"NY_0week_beforepeak_long_projection.csv"))

    ylim_=range(as.numeric(c(table_est_death,
               table_l_est_death,table_h_est_death)))
    par(mar=c(1,1,1,1))
    pdf(paste0(dir_fig,country,"_",data_sourse,"_death_long_pro_0week_NY.pdf"))
  ...
  for(i in 15){#MH
    ...
    # dates for plot - part of fitting + projection
    table_dates=seq.Date(from=as.Date("2020-03-14"),to=as.Date("2020-07-15"),"day")
    ...
    #for plot
    date_training_plot=date_training[which(!as.Date(date_training,format="%Y-%m-%d")< 
                                             as.Date("2020-03-14",format="%Y-%m-%d"))]
    ...
    load("/Users/yiwang/Dropbox/cvd/cvd_N/model/data/df_mh.RData")#df_gj
    
    df_death_country=df_mh#[which(!as.Date(df_death_country$DateRep,format="%d/%m/%Y")<
                          #                    as.Date("2020-03-14",format="%Y-%m-%d")),]
    #df_death_country=df_death_country[which(!as.Date(df_death_country$DateRep,format="%d/%m/%Y")>
    #                                          as.Date("2020-07-15",format="%Y-%m-%d")),]
    reported_deaths_country=df_death_country$Deaths
    ...
    write.csv(table,file=paste0(dir_fig,"GJ_projection_only.csv"))
    
    ylim_=range(as.numeric(c(table_est_death,
                             table_l_est_death,table_h_est_death)))
    par(mar=c(1,1,1,1))
    pdf(paste0(dir_fig,country,"_",data_sourse,"_death_long_projection_only.pdf"))
    plot(table_true_death[(table_true_death)!="NA"],ylim=ylim_,xlim=c(0,length(table_est_death)))
    lines(table_est_death,col="red")
    lines(table_l_est_death,col="red")
    lines(table_h_est_death,col="red")
