...
d_italy=d_italy[which(as.Date(d_italy$DateRep,format="%d/%m/%Y") < as.Date("2020-03-21",format="%Y-%m-%d")),]
...
df_case=df_case[which(as.Date(df_case$DateRep,format="%d/%m/%Y")<as.Date("2020-05-02")),]
# > head(df_case)
# Cases Countries.and.territories day year month    DateRep Deaths
...
N2 = 130 # increase if you need more forecast
...
save(out_cvd19_model,file="output/out_cvd19_model_italy_ny_1week_constraint_4chains.RData")
# 
...
data_sourse_='jhu'
dir_figure="fig/ita_ny_1week_constraint_4chains/"
...
for(i in 2){#italy
  ...
  table_dates=seq.Date(from=as.Date("2020-03-02"),to=as.Date("2020-05-15"),"day")
  ...
  #for plot
  date_training_plot=date_training[which(!as.Date(date_training,format="%Y-%m-%d")< as.Date("2020-03-02",format="%Y-%m-%d"))]
  ...
  estimated.deaths=estimated.deaths[which(! as.Date(date_training_projection,format="%Y-%m-%d") < as.Date("2020-03-02",format="%Y-%m-%d"))]
  #variance of estimated death
  var_est_deaths=colVars(out_model$E_deaths_all[,,i])
  var_est_deaths=var_est_deaths[which(! as.Date(date_training_projection,format="%Y-%m-%d") < as.Date("2020-03-02",format="%Y-%m-%d"))]
  ...
  df_death_country=df_death_country[which(!as.Date(df_death_country$DateRep,format="%d/%m/%Y")<as.Date("2020-03-02",format="%Y-%m-%d")),]
  df_death_country=df_death_country[which(!as.Date(df_death_country$DateRep,format="%d/%m/%Y")>as.Date("2020-05-15",format="%Y-%m-%d")),]
  ...
  write.csv(table,file=paste0(dir_fig,"italy_1week_constraint_4chains.csv"))
}

ylim_=range(as.numeric(c(table_est_death,
                         table_l_est_death,table_h_est_death)))
par(mar=c(1,1,1,1))
pdf(paste0(dir_fig,"italy1week_constrain_4chains.pdf"))
...
for(i in 14){#NY
  ...
  # dates for plot - part of fitting + projection
  table_dates=seq.Date(from=as.Date("2020-03-18"),to=as.Date("2020-05-15"),"day")
  ...
  df_death_country=df_death_country[which(!as.Date(df_death_country$DateRep,format="%d/%m/%Y")<
                                            as.Date("2020-03-18",format="%Y-%m-%d")),]
  df_death_country=df_death_country[which(!as.Date(df_death_country$DateRep,format="%d/%m/%Y")>
                                            as.Date("2020-05-15",format="%Y-%m-%d")),]
  ...
  write.csv(table,file=paste0(dir_fig,"NY_1week_constraint_4chains.csv"))
  
  ylim_=range(as.numeric(c(table_est_death,
                           table_l_est_death,table_h_est_death,table_true_death[(table_true_death)!="NA"])))
  par(mar=c(1,1,1,1))
  pdf(paste0(dir_fig,country,"_",data_sourse,"_1week_NY_constraint_4chains.pdf"))
  plot(table_true_death[(table_true_death)!="NA"],ylim=ylim_,xlim=c(0,length(table_est_death)))
  lines(table_est_death,col="red")
  lines(table_l_est_death,col="red")
  lines(table_h_est_death,col="red")
